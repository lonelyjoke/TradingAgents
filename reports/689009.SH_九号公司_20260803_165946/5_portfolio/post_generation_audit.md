...
- BLOCKED: blocking_errors=1, research_errors=6, warnings=15.
...
| sell_side_expectation_usage | warning | review only | sell-side forecast/valuation observations are absent from the PM expectation-gap analysis: KSI09 |
| sell_side_expectation_lineage | warning | review only | PM expectation matrix cites KSI ids absent from the deterministic sell-side ledger: KSI01, KSI03, KSI04 |
| handoff_numeric_consistency | error | blocks formal publication | Portfolio Manager silently changed current dilutedshares from 735.8485 mn shares to 7.36 mn shares |