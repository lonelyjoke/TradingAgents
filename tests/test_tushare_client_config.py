...
def test_fallback_env_loader_overrides_stale_process_token(monkeypatch, tmp_path):
    ...
    env_file.write_text("TUSHARE_TOKEN=" + "y" * 64 + "\n", encoding="utf-8")
    ...
    assert tushare_client.get_tushare_token() == "y" * 64


def test_short_tushare_token_fails_before_api_call(monkeypatch):
    monkeypatch.setattr(tushare_client, "_load_env", lambda: None)
    monkeypatch.setenv("TUSHARE_TOKEN", "x" * 56)
    ...
    try:
        ...
        assert "length is 56" in str(exc)
