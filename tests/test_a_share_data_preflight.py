...
def test_a_share_preflight_keeps_running_when_daily_basic_unavailable(monkeypatch):
    monkeypatch.setattr(a_share_preflight, "_fetch_stock_basic", lambda symbol: {"name": "中国平安", "industry": "保险"})
    monkeypatch.setattr(a_share_preflight, "_fetch_daily_with_backfill", _ready_daily)
    monkeypatch.setattr(
        a_share_preflight,
    ...
    assert "| daily_basic | failed |" in rendered
    assert "read timeout" in rendered
    assert "Status: partial" in rendered
    assert "do not stop report generation" in rendered
...
def test_a_share_preflight_keeps_running_when_statement_data_missing(monkeypatch):
    monkeypatch.setattr(a_share_preflight, "_fetch_stock_basic", lambda symbol: {"name": "中国平安", "industry": "保险"})
    monkeypatch.setattr(a_share_preflight, "_fetch_daily_with_backfill", _ready_daily)
    monkeypatch.setattr(
        a_share_preflight,
    ...
    assert "| income | failed | no rows returned |" in rendered
    assert "unavailable checks: income" in rendered


def test_a_share_preflight_keeps_running_when_filing_text_missing(monkeypatch):
    ...
    assert "| filing_text | failed |" in rendered
    assert "no readable annual/semiannual/quarterly report text returned" in rendered
    assert "mock filing text audit" in rendered


def test_a_share_preflight_retains_explicit_strict_mode(monkeypatch):
    monkeypatch.setattr(a_share_preflight, "_fetch_stock_basic", lambda symbol: {"name": "中国平安", "industry": "保险"})
    monkeypatch.setattr(a_share_preflight, "_fetch_daily_with_backfill", _ready_daily)
    monkeypatch.setattr(
        a_share_preflight,
        "_fetch_daily_basic_latest",
        lambda symbol, curr_date: (_ for _ in ()).throw(TushareDataError("read timeout")),
    )
    monkeypatch.setattr(a_share_preflight, "_fetch_fina_indicator", _ready_frame)
    monkeypatch.setattr(a_share_preflight, "_fetch_income_statement_data", _ready_frame)
    monkeypatch.setattr(a_share_preflight, "_fetch_balance_sheet_data", _ready_frame)
    ...
    with pytest.raises(AShareDataPreflightError):
        run_a_share_data_preflight(
            "601318.SH",
            "2026-06-03",
            selected_analysts=["market", "fundamentals"],
