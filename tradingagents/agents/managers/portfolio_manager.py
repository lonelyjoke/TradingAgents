...
- Do not map uncertainty, a starter position, or waiting for one disclosure mechanically to Hold. Derive the rating from the reconciled scenario expected value and downside distribution. Put evidence insufficiency in `research_readiness`, confidence and sizing; Hold is valid only when the completed underwriting model itself supports a roughly balanced current risk/reward.
