...
- Copy each shared `UWQ` id into the matching `question_verdicts.question_id`; do not rename or silently drop a decisive packet question. Populate `evidence_utilization_ledger` for every evidence id attached to a decisive underwriting question or thesis bridge. Each id receives exactly one decision: `adopted`, `rejected`, `watch`, or `conflict_unresolved`. Adopted evidence must state the named old->new model/probability/value effect; rejected evidence needs a reason; watch/unresolved evidence needs a dated verification gate. Resolve source conflicts by filing/official fact -> reproducible calculation -> analyst estimate -> channel/proxy precedence, and record the resolution rather than blending incompatible facts.
...
- Before accepting any segment-mix margin bridge, recalculate `sum(segment revenue weight x segment margin)` and require weights to reconcile to 100% and the result to reconcile to consolidated gross margin. If official segment margins are missing, keep them null or explicitly illustrative and anchor the forecast on the reported/consolidated margin; never invent or plug a residual segment margin merely to force closure, and never claim the consolidated margin is proven by an unreconciled segment equation.
...
- Treat `research_dossier` as the reader-led organizing spine: reconcile the debate against its decisive questions, industrial-chain transmission, profit pools, forecast spine and seven chapter packets. The accepted model must make those chapters answerable; do not organize the verdict as a recap of upstream agents.
- If Knowledge Planet context is available, keep a **Knowledge Planet Intelligence Verdict** explicit enough to use the Single-Stock Knowledge Fusion Pack and dossier routing first, separate information-rich industry data/channel checks/research feedback from sell-side promotion, and reconcile private/proxy clues with filings, Tushare data, peer evidence, price/volume behavior, and official announcements. Preserve the routed role for every KPE: numeric model input, probability adjustment, dated verification item, or rejected. Decide whether it upgrades the catalyst/expectation gap, only creates a watch item, or raises pump/crowding risk.
- If a foreign sell-side forecast enters the accepted underwriting model, label it as foreign-sell-side-anchored rather than independent. Preserve the institution and date, the exact forecast lines adopted, whether adoption is direct/partial/probability-only, and characterize the forecast as optimistic/balanced/conservative/mixed using its demand, ASP, share, margin, cash-flow and valuation assumptions relative to filings, public evidence, other brokers and the pre-adoption TradingAgents model. State the before->after model impact and the dated evidence that can verify or falsify the broker case.
