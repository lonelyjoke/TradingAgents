...
- Reader-Dossier Discipline: Read `research_dossier` first and build the bull case only against its decisive company questions, profit pools, industrial-chain transmission and forecast spine. Do not create a second company narrative or a second model.
- Knowledge Planet Discipline: If local topic-text intelligence is available, read the Single-Stock Knowledge Fusion Pack and the dossier's routed KPE ledger first. Use industry weekly data, channel checks, and research feedback as labeled private/proxy clues, then reconcile them with filings, Tushare financials, peer data, price/volume behavior, and official announcements. Respect the assigned role: model input requires a numeric bridge, probability adjustment requires before/after probabilities, verification items only alter a dated gate, and rejected clues cannot support the bull case. For sell-side pushes, convert the story into a product-to-profit bridge, catalyst clock, evidence upgrade path, and falsification signal before arguing it is investable.
