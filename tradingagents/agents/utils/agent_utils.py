...
def get_research_gap_instruction() -> str:
    ...
    return (
        " Missing-data neutrality (highest-priority rule): when a thesis-critical driver is missing "
        ...
        "analysis collapse into technical indicators or static PE/PB alone. A failed "
        "retrieval is neutral non-evidence and neutral for direction: it must never mechanically lower or raise "
        "the rating, conviction tier, position size, valuation multiple, or publication "
        "status. Explain why the field matters, leave unsupported numeric cells null, "
        "build conditional scenarios from the verified evidence that remains, and list "
        "the exact source/retrieval task that should be checked next. Keep the investment "
        "direction based only on verified positive and negative evidence and expected value; "
        "missing data alone cannot justify Underweight/Sell without independent verified evidence. "
        "Do not default to Hold merely because a source or field is unavailable."
    )
...
def get_knowledge_planet_instruction() -> str:
    ...
    return (
        " Knowledge Planet discipline: treat locally imported topic-text context "
        "as alternative intelligence and sell-side/channel perspective, not as a "
        "replacement for official filings, announcements, financial statements, "
        "market data, or reputable news. Preserve the source label: industry weekly "
        "data, channel checks, company research feedback, PDF sell-side frameworks, "
        "sell-side push, or rumor/sentiment. Combine it with objective evidence in "
        "a strict evidence ladder: official filings/announcements and financials set "
        "the baseline; price/volume and reputable news verify market confirmation; "
        "Knowledge Planet adds expectation-gap hypotheses, industry color, KPI "
        "frameworks, and hard-to-publicly-verify clues. Knowledge Planet attachment "
        "PDFs, images, audio and OCR are intentionally outside this pipeline; never "
        "infer their contents from an attachment title or URL. Do not discard industry data "
        "or research feedback merely because it is not public; instead use it as a "
        "labeled clue and define the next observable proxy. For promoted names, force "
        "the story-to-profit bridge: product or business driver -> revenue/margin/FCF "
        ...
        "expectation gap. If Knowledge Planet conflicts "
...
def get_baijiu_instruction() -> str:
    ...
    return (
        ...
        "label it as a neutral retrieval task. Do not mechanically alter the rating "
        "or fabricate a relative-allocation claim from the failed peer basket."
    )
...
def get_consumer_staples_instruction() -> str:
    ...
    return (
        ...
        "through evidence is a neutral retrieval task, not permission to write a "
        "generic consumer-stock thesis or mechanically alter the rating."
    )
...
def get_optical_module_instruction() -> str:
    ...
    return (
        ...
        "is a neutral retrieval task and must not mechanically alter the rating. Treat CPO/LPO/silicon photonics, "
...
def get_insurance_instruction() -> str:
    ...
    return (
        ...
        "investment-yield, COR, or channel-quality evidence is a neutral research "
        "gap with an explicit retrieval task, not an automatic rating adjustment. For insurance or high-dividend defensive "
...
def get_medical_device_instruction() -> str:
    ...
    return (
        ...
        "pull-through evidence is a neutral research gap and must not mechanically alter the rating."
    )
...
def get_metals_mining_instruction() -> str:
    ...
    return (
        ...
        "If these tables cannot be filled from evidence, state that the gap is neutral for direction and "
        "retrieval task rather than replacing them with a generic PE paragraph. "
        "Missing reserve, grade, equity-output, AISC, project-ramp, hedging, or NAV "
        "evidence must not mechanically alter rating, conviction, or sizing and cannot "
        "prove deterioration or improvement by itself. "
        ...
        "missing, treat the gap as neutral non-evidence with a retrieval task; do not "
...
def get_buy_side_thesis_instruction() -> str:
    ...
    return (
        ...
        "expectation gap is weak, or probability/payoff is not attractive. When Hold "
        "is selected, label the posture as positive watch, defensive starter, neutral "
        "wait, or negative watch so that Hold does not hide the intended trade. If writing "
...
def get_earnings_model_instruction() -> str:
    ...
    return (
        ...
        "Before declaring a model field missing, apply deterministic completion: total "
        "shares may be calculated from Tushare total_share (10,000-share unit), market "
        "capitalization/current price, or parent profit/reported EPS with the formula and "
        "period shown; capex may be calculated from cash paid to acquire or construct long-"
        "term assets; FCF may be calculated as OCF minus consistently defined capex. Mark "
        "these rows calculated, cross-check independent routes, and never confuse a derived "
        "value with a reported fact. "
...
def get_question_led_debate_instruction() -> str:
    ...
    return (
        ...
        "Before writing a directional story, select the 3-5 questions that can "
        ...
        "outputs should include evidence-weighted question verdicts when the "
        "agenda is supplied. Do not paste the upstream table mechanically; "
        "rewrite it as an investment committee issue log. Treat precomputed modules "
        "as an evidence library, not an outline: a module earns space only when it "
        "changes a named model line, scenario probability, valuation conclusion, "
        "counterargument, or dated verification task. Reconcile conflicts across "
        "sources instead of writing one paragraph per module. If a thesis-critical "
