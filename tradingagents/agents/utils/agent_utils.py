...
def get_buy_side_underwriting_modules_instruction() -> str:
    ...
    return (
        ...
        "(8) Value-stock safety price: for blue chips, mature value stocks, banks, "
        "cash-flow compounders, or defensive dividend names with resilient balance "
        "sheets, estimate a conservative safety price or safety price band for "
        "slow accumulation by new builders. This is not a target price and not a "
        "stop-loss; it is the price where normalized low-cycle EPS/FCF, sustainable "
        "dividend yield, book value/PB and ROE, net cash or leverage, asset quality, "
        "cash conversion, payout capacity, and historical/peer valuation floors "
        "together create a margin of safety. Explain why a temporary break below "
        "that level should be mean-reverting if the financial state holds, and "
        "name the financial deterioration that would invalidate the safety price. "
        "Do not assign a safety price to structurally declining, highly leveraged, "
        "deep cyclical, concept-driven, or evidence-thin companies; say no reliable "
        "safety price can be set. "
