...
9. `research_readiness=ready` only when material segments, three-year consolidated model, scenario valuation, periods/units and decisive evidence are sufficiently complete. Otherwise use partial or blocked and explain why.
...
12. Distinguish a descriptive module from a decision-useful one. Do not promote a module into the packet unless it changes a named forecast line, scenario probability, valuation bucket, confidence cap or dated verification task.
