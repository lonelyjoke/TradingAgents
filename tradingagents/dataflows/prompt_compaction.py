...
_IMPORTANT_RE = re.compile(
    "|".join(
        [
            ...
            r"\b(consumer staples|food|beverage|frozen|prepared dish|surimi|restaurant traffic|distributor|promotion|channel inventory|sell-through|raw material)\b",
            r"\b(optical module|optical communication|datacom|transceiver|800G|1\.6T|400G|CPO|LPO|silicon photonics|coherent optics|hyperscaler|AI capex|cloud capex|customer qualification)\b",
            r"\b(building materials|cement|clinker|waterproof|gypsum|fiberglass|glass|coating|ceramic|pipe)\b",
            r"\b(biopharma|biotech|clinical|ClinicalTrials|FDA|NMPA|CDE|EMA|PDUFA|NDA|BLA|Phase|trial|ORR|PFS|OS|DOR|pipeline|BRUKINSA|TEVIMBRA|zanubrutinib|tislelizumab|sonrotoclax)\b",
            r"\b(software|SaaS|ARR|MRR|ARPU|MAU|paid user|renewal|churn|NRR|GRR|seat|subscription|deferred revenue|contract liability|WPS|AI monetization|Copilot|Xinchuang)\b",
            r"\b(insurance|insurer|NBV|embedded value|P/EV|solvency|COR|loss ratio|expense ratio|persistency|surrender|bancassurance|agent productivity|CSM|NCSM)\b",
            r"\b(medical device|IVD|reagent|consumable|installed base|tender|procurement|VBP|volume-based procurement|NMPA|FDA|CE|ultrasound|endoscopy|monitoring|anesthesia|ventilator|CT|MRI|registration|overseas channel)\b",
            r"\b(metals|mining|miner|smelting|refining|reserve|resource|grade|equity output|AISC|cash cost|mine life|NAV|SOTP|COMEX|LME|LBMA|SHFE|GFEX|TC/RC|hedging|derivatives)\b",
            r"\b(price[- ]move|drawdown|selloff|sharp drop|attribution|residual|same[- ]metal|cross[- ]metal|failed rebound|emotion kill|risk[- ]premium|turnover|volume ratio)\b",
            r"\b(intraday|minute[- ]line|minute K|minute bar|first 30min|last 30min|opening phase|late-session|volume concentration|liquidity)\b",
            r"\b(relative strength|excess return|benchmark|correlation|beta|style index|industry proxy)\b",
            r"\b(order|backlog|inventory|capacity|utilization|price|spread)\b",
            r"\b(ASP|GWh|shipment|installation|contract liabilit|working capital|target price|entry band|SOTP)\b",
            ...
            r"\b(earnings guidance|performance preview|performance preannouncement|quick report|official guidance|company announcement)\b",
            "(\u4e1a\u7ee9\u9884\u544a|\u4e1a\u7ee9\u5feb\u62a5|\u9884\u589e|\u9884\u51cf|\u626d\u4e8f|\u7eed\u76c8|\u5f52\u6bcd|\u6263\u975e|\u57fa\u672c\u6bcf\u80a1\u6536\u76ca)",
            ...
            r"(画像|异质性|收入|利润|毛利|现金流|负债|订单|库存|产能|价格|价差)",
            r"(相对走势|相对强弱|指数联动|超额收益|相关性|贝塔|跑赢|跑输)",
...
_BASE_LIMITS = {
    ...
    "price_move_attribution_context": 5000,
    "intraday_behavior_context": 3500,
    "relative_strength_context": 5000,
    "shipping_context": 4500,
    "peer_comparison_context": 3500,
    "supply_chain_comparison_context": 3000,
    "earnings_model_context": 3500,
    "company_events_context": 5000,
    ...
    "knowledge_planet_context": 7000,
    "baijiu_context": 5500,
    "compute_leasing_context": 4500,
    "dividend_defensive_context": 5000,
    "building_materials_context": 5500,
    "consumer_staples_context": 6000,
    "optical_module_context": 6000,
    "biopharma_context": 5500,
    "software_context": 5500,
    "insurance_context": 5500,
    "medical_device_context": 5500,
    "metals_mining_context": 6000,
    "industry_cycle_context": 3500,
    "company_business_model_context": 5500,
