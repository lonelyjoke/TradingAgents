...
def build_quality_audit_context(
    ...
    *,
    ...
    missing_forecast_contracts = [
        marker for marker in forecast_contract_markers if marker not in forecast_model_context.lower()
    ]
    if missing_forecast_contracts:
    ...
    rows = [
        ...
        (
            "Segment-level prosperity framework",
            "ready" if "Segment-Level Prosperity Underwriting Contract" in company_business_model_context else "partial",
            "material businesses must be classified separately by prosperity level, marginal direction, evidence, counterevidence, and EPS/FCF transmission",
        ),
